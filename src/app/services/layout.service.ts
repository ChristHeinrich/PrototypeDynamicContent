...
// import { Layout } from '../types/appsDescription';
...
export class LayoutService {
  ...
  private appsDescription: App[] = [];
  constructor(private messageService: MessageService, private http: HttpClient) { }

  getDsl(): Observable<App[]> {
    return of(this.appsDescription);
  }

  setCachedLayout(apps: App[]): void {
    this.appsDescription = apps;
  }

  fetchLayout(): Observable<App[]> {
    this.messageService.add('LayoutService: fetchted appsDescription from ' + this.dslUrl);
    return this.http.get<any[]>(this.dslUrl)
      .pipe(
        delay(100),
        tap(_ => this.log('fetched appsDescription'))
